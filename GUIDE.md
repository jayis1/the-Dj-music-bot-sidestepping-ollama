...
- **"Already playing audio" race condition** — Discord's `VoiceClient.play()` raises `ClientException: Already playing audio` when called while audio is still active. This happened in multiple code paths: soundboard playing over DJ sound effects, DJ sounds starting while bed music was playing, AI side host speaking while previous audio was still active, and song playback racing with lingering TTS/sounds. Three fixes: (1) `_dispatch_audio_play()` — the central audio injection point — now checks `vc.is_playing()`, calls `vc.stop()`, and waits 150ms for Discord's VoiceClient to process the stop before calling `vc.play()`. (2) `_on_tts_done()` no longer starts bed music simultaneously with sound effects — sound effects take priority, and bed music only starts after sounds finish. (3) The soundboard web API explicitly stops bed music before playing a sound effect.

- **Danish voices** — Added `da_female.wav` and `da_male.wav` to `assets/moss_voices/` for Danish-language DJ and AI side host. The `MOSS_VOICE_CATALOG` and `EDGE_VOICE_BY_LANG` mapping ensure proper voice resolution across all TTS engines: MOSS uses the `.wav` files directly, and edge-tts fallback maps `da_*` voices to `da-DK-SofieNeural` (female) and `da-DK-JeppeNeural` (male) instead of falling back to English voices.
