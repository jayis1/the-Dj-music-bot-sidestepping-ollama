...
from utils.llm_dj import OLLAMA_DJ_AVAILABLE
...
def api_play(guild_id):
    ...
    async def _play():
        ...
        if (
            vc
            and not getattr(vc, "is_playing", lambda: False)()
            and not getattr(vc, "is_paused", lambda: False)()
        ):
...
def api_ytcookies_inject():
    ...
    if isinstance(extension_cookies, list) and source == "extension":
        ...
        for c in extension_cookies:
            ...
            "TRUE" if c.get("httpOnly", False) else "FALSE"
    ...
    # ── Handle text-based cookie formats (paste / snippet) ──
        ...
        if "\t" in cookie_text and ("TRUE" in cookie_text or "FALSE" in cookie_text):
            ...
            any("youtube" in line.lower() for line in lines)
...
def api_save_stream_key():
    ...
    try:
        ...
        logging.info(
            f"Dashboard: YouTube stream key saved to .env (hint: ...{stream_key[-4:]})"
        )
        return jsonify({"ok": True})
...
def api_youtube_stream_toggle(guild_id):
    ...
    if music._yt_stream_active:
        ...
        # ── Mirror mode: requires Discord voice connection ──
        if mode == "mirror":
            guild = bot.get_guild(guild_id)
            if not guild or not guild.voice_client:
                return jsonify(
            ...
            async def _start_mirror():
                ...
                logging.info(
                    f"YouTube Live: Mirror stream started via Mission Control for guild {guild_id}"
                )

            asyncio.ensure_future(_start_mirror(), loop=bot.loop)
...
def api_youtube_stream_skip(guild_id):
    ...
    if "loop" in data:
        streamer._playlist_loop = bool(data["loop"])
        changes.append(f"loop={'on' if streamer._playlist_loop else 'off'}")

    if "shuffle" in data:
        streamer._shuffle = bool(data["shuffle"])
        changes.append(f"shuffle={'on' if streamer._shuffle else 'off'}")

    if data.get("playlist_url"):
        new_url = data["playlist_url"].strip()
        streamer._playlist_url = new_url
        changes.append("playlist updated")

        # Reload playlist in background
        async def _reload():
            await streamer._load_playlist()
            streamer._playlist_index = 0
            logging.info(f"YouTube Live: Playlist reloaded: {new_url[:80]}")

        asyncio.ensure_future(_reload(), loop=bot.loop)

    if data.get("reload"):

        async def _reload():
            await streamer._load_playlist()
            logging.info("YouTube Live: Playlist reloaded (refresh)")

        asyncio.ensure_future(_reload(), loop=bot.loop)
        changes.append("playlist refreshed")
        ...
        return jsonify(
            {
                "connected": False,
                "enabled": False,
                "error": "OBS integration not configured",
            }
        )
...
def api_obs_streaming_configure():
    ...
    data = request.json or {}
    stream_key = data.get("stream_key", "").strip() or getattr(
    ...
    getattr(
        config, "YOUTUBE_STREAM_BACKUP_URL", "rtmp://b.rtmp.youtube.com/live2?backup=1"
    )
    ...
    if not stream_key:
        return jsonify(
            {
                "ok": False,
                "error": "No stream key configured. Set it on the OBS page or in .env.",
            }
        )
    ...
    if result.get("error") and not result.get("connected"):
        return jsonify(
            {
                "ok": False,
                "error": f"Failed to configure OBS: {result.get('error', 'unknown error')}",
            }
        )
...
def api_obs_streaming_configure_and_start():
    ...
    if not bridge or not bridge.enabled:
        return jsonify(
            {
                "ok": False,
                "error": "OBS not configured. Enable OBS_WS_ENABLED and set OBS_WS_PASSWORD in .env.",
            }
        )
    ...
    if not status.get("connected"):
        return jsonify(
            {
                "ok": False,
                "error": "OBS is not connected. Make sure OBS Studio is running with WebSocket server enabled.",
            }
        )
    ...
    if status.get("streaming"):
        return jsonify(
            {
                "ok": True,
                "message": "OBS is already streaming.",
    ...
    logging.info(
        f"OBS Mission Control: Configuring stream → {rtmp_server} (key: ...{stream_key[-4:]})"
    )
    result = bridge.set_stream_settings(
        service="rtmp_custom",
        server=rtmp_server,
    ...
    if result.get("error") and not result.get("connected"):
        return jsonify(
            {
                "ok": False,
                "error": f"Failed to configure OBS stream settings: {result.get('error', 'unknown')}",
            }
        )
    ...
    if overlay_mode in ("browser", "auto"):
        try:
            ...
            if result and not result.get("error"):
                ...
                logging.info(
                    "OBS Mission Control: Browser overlay created (full overlay + visualizer)"
                )
        except Exception as e:
            if overlay_mode == "browser":
                logging.warning(f"OBS Mission Control: Browser overlay failed: {e}")
            else:
                logging.debug(
                    f"OBS Mission Control: Browser overlay not available ({e}), falling back to native"
                )
    ...
    if not browser_overlay_created:
        ...
        if overlay_mode == "browser":
            logging.warning(
                "OBS Mission Control: Browser overlay requested but failed — using native fallback"
            )
    ...
    if start_result.get("error"):
        ...
        return jsonify(
            {
                "ok": False,
                "error": f"Failed to start OBS streaming: {start_result.get('error', 'unknown')}",
            }
        )
...
def api_obs_set_scene():
    ...
    if not bridge or not bridge.enabled:
        return jsonify({"ok": False, "error": "OBS not configured"})
    scene_name = request.json.get("scene_name", "") or request.form.get(
        "scene_name", ""
    )
    if not scene_name:
...
def _hermes_require_auth():
    ...
    if not _hermes_auth():
        ...
        if not api_key:
            return jsonify(
                {"error": "Hermes API disabled — set HERMES_API_KEY in .env"}
            ), 403
        return jsonify(
            {"error": "Unauthorized — provide Authorization: Bearer <HERMES_API_KEY>"}
        ), 401
    return None  # Auth OK
        ...
        queue_list.append(
            {
                ...
                or getattr(item, "original_url", None),
    ...
    return jsonify(
        {
            ...
            "current_song": {
                ...
                or getattr(current, "original_url", None),
...
def hermes_queue_remove():
    ...
    if position >= len(items):
        return jsonify(
            {
                "ok": False,
                "error": f"Position {position} out of range (queue has {len(items)} items)",
            }
        ), 400
...
def hermes_queue_move():
    ...
    if from_pos < 0 or to_pos < 0:
        return jsonify(
            {"ok": False, "error": "Both 'from' and 'to' positions required (≥0)"}
        ), 400
    ...
    if from_pos >= len(items) or to_pos >= len(items):
        return jsonify(
            {
                "ok": False,
                "error": f"Position out of range (queue has {len(items)} items)",
            }
        ), 400
...
def hermes_queue_shuffle():
    ...
    if not q or len(q._queue) < 2:
        return jsonify(
            {"ok": True, "shuffled": 0, "message": "Queue too short to shuffle"}
        )
...
def hermes_autodj_toggle():
    ...
    logging.info(
        f"Hermes: Auto-DJ {status} for guild {gid} (source={source[:80] if source else 'none'})"
    )
...
def hermes_autodj_fill():
    ...
    source_used = music.autodj_source.get(gid, "") or getattr(
    ...
    try:
        ...
        future = asyncio.run_coroutine_threadsafe(
            music._autodj_fill(fake_ctx), bot.loop
        )
...
def hermes_cookies_inject():
    ...
    if isinstance(extension_cookies, list):
        lines = ["# Netscape HTTP Cookie File"]
        lines.append(
            f"# Injected by Hermes Agent at {time.strftime('%Y-%m-%d %H:%M:%S')}"
        )
        for c in extension_cookies:
            if not isinstance(c, dict):
                continue
    ...
    try:
        with open(cookiefile, "w") as f:
            f.write(cookie_text)
        logging.info(
            f"Hermes: Cookie file written → {cookiefile} ({len(cookie_text)} bytes, source={source})"
        )
    except Exception as e:
        return jsonify({"ok": False, "error": f"Failed to write cookie file: {e}"}), 500
...
def hermes_silverbullet_incident():
    ...
    if page_path:
        ...
        return jsonify(
            {"ok": False, "error": "SilverBullet not configured or write failed"}
        ), 503
...
def hermes_silverbullet_session_end():
    ...
    if ok:
        ...
        return jsonify(
            {
                "ok": False,
                "error": "Failed to update session (page not found or write failed)",
            }
        ), 503
